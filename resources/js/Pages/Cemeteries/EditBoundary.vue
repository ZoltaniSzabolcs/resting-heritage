...
onMounted(() => {
    ...
    // Engedélyezzük a poligon rajzolást
    map.pm.addControls({
        position: 'topleft',
        drawPolygon: true,
    ...
    // Ha már van boundary, kirajzoljuk
    if (cemetery.boundary) {
        drawnLayer = leaflet.geoJSON(cemetery.boundary).addTo(map);
        map.fitBounds(drawnLayer.getBounds());
    }

    // 1️⃣ Új poligon létrehozása
    map.on('pm:create', e => {
        if (drawnLayer) map.removeLayer(drawnLayer);
        drawnLayer = e.layer;
    });

    // 2️⃣ Poligon szerkesztése
    map.on('pm:edit', e => {
        drawnLayer = e.layer; // új geometriát kap
    });

    // 3️⃣ Poligon elmozgatása
    map.on('pm:dragend', e => {
        drawnLayer = e.layer;
    });
